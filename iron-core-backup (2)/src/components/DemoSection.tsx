                  ...
                  {filteredFoods.length > 0 ? (
                    filteredFoods.map((food, i) => (
                      <button
                        ...
                        className="w-full px-5 py-4 text-left hover:bg-zinc-700 transition-colors flex items-center justify-between group border-b border-white/5 last:border-0"
                      >
                        <div className="flex-1 min-w-0 pr-4">
                          <div className="flex items-center gap-2 mb-0.5">
                            <p className="font-bold text-white group-hover:text-red-500 transition-colors text-sm truncate">{food.dishName}</p>
                            <span className="shrink-0 px-1.5 py-0.5 rounded bg-zinc-800 text-[8px] text-zinc-400 uppercase tracking-widest border border-white/10 group-hover:border-red-500/30 transition-colors">{food.cuisineType}</span>
                          </div>
                          {food.dietaryTags && food.dietaryTags.length > 0 && (
                            <div className="flex flex-wrap gap-1.5 mt-1.5">
                              {food.dietaryTags.map(tag => (
                                <span key={tag} className="text-[9px] font-medium px-1.5 py-0.5 rounded-sm bg-black/30 text-zinc-400 border border-white/5">
                                  {tag}
                                </span>
                              ))}
                            </div>
                        ...
                        <div className="w-8 h-8 rounded-full bg-black/30 flex items-center justify-center shrink-0 group-hover:bg-red-500/20 transition-colors">
                          <Plus className="w-4 h-4 text-zinc-500 group-hover:text-red-500 transition-all" />
                        </div>
                      </button>
                    ))
                  ) : (
